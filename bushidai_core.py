...
class HyperonSimulator:
    ...
    def cognitive_cycle(self, natural_goal):
        ...
        self.cycle_count += 1
        rule = llm_grounder.ground(natural_goal)
        print(Colors.OKBLUE + f"Hyperon MeTTa Grounding → Rule: {rule}" + Colors.END)
        tv = neural.perceive(self.fixed_state)
        print(Colors.OKCYAN + f"Neural perceive → TV: {tv:.4f}" + Colors.END)
        inference_tv = 0.5023
        print(Colors.OKGREEN + f"PLN inference TV: {inference_tv:.4f}" + Colors.END)
...
if __name__ == "__main__":
    ...
    for goal in args.goals:
        if "verifiable credential" in goal.lower() or "vc" in goal.lower():
            sim.vc_cognitive_cycle(goal)
        else:
